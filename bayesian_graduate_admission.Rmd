...
1 & \text { if } &  \text { Chance of admit > 0.65 } \\
0 & \text { if } & X_{j} \text { Chance of admit} \leq \text{0.65 }
\end{array}\right.$$

So we can now assume:

$$ Y_i|\pi_i \stackrel{ind.}{\sim} Ber(\pi_i) \quad i=1,....,n $$
Now the aim is to model the effect of predictors on $\pi_i$, but we can't do it directly because then we would have a problem with negative and above 1 probabilities. So we will model $\eta = g(p)$, where $g(.)$ is a *link function*, such that $0 \leq h(\eta) \le 1$.
...
In the following code, with the help og JAGS, we perform a Metropolis Hastings with 5000 iteration, no *burnin period* and *thinning* equal to 1.
...
As output we obtain a [5000 x 375] matrix, where the columns corresponds to our $\beta$ coefficients, and each row is the output of an iteration of the Metropolis Hastings, *i.e.* $\boldsymbol{{\beta}}^{s}$
...
As new observation we consider the `x_star` data frame, that we created at the beginning by splitting the original dataset in train and test. It contains 125 new observations: $\boldsymbol{x}^*_j =(x_{j,1}^*),....,(x_{j,11}^*)$ for $j = 1,...,125$
...
```
Now that we have 5000 values for $\pi_i$, we can sample from a bernoulli which has it as parameter and obtain the "fitted" values of $y_i$ as the mode of the distribution of each future observation.
...
**How can we use this kind of results to do our predictions?**
...
We can now evaluate how good is our model in prediction, comparing our prediction with the true values contained in `y_test`
